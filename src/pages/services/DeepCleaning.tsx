...
export default function DeepCleaning() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 overflow-x-hidden relative">
      ...
      {/* Testimonials Section — real reviews only, managed from the admin dashboard */}
      {storeData?.testimonials && (storeData.testimonials as any[]).filter((t: any) => t.isPublished).length > 0 && (
              ...
              {(storeData.testimonials as any[])
                ...
                .map((t: any) => {
                  const initials = t.authorName
                    ...
                    .toUpperCase();
                  return (
                    <div key={t.id} className="bg-slate-50/50 p-6 rounded-2xl border border-slate-100 shadow-sm relative flex flex-col justify-between hover:shadow-md transition-shadow">
                      <div>
                        <div className="flex justify-center mb-4 relative">
                          {t.avatarUrl ? (
                            ...
                            <div className="w-14 h-14 rounded-full bg-emerald-600 flex items-center justify-center text-white font-bold text-lg shadow-inner">
                              {initials}
                            </div>
                          )}
                        </div>
                        <div className="text-center mb-3">
                          <p className="font-bold text-slate-800 text-sm">{t.authorName}</p>
                          {t.authorRole && <p className="text-[10px] text-slate-400 font-medium">{t.authorRole}</p>}
                        </div>
                        <div className="flex items-center justify-center gap-1 mb-4">
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4, 5].map(i => (
                              <Star key={i} size={14} className={i <= t.rating ? "text-amber-400 fill-current" : "text-slate-200 fill-current"} />
                            ))}
                          </div>
                        </div>
                        <p className="text-slate-600 text-xs leading-relaxed text-center italic">"{t.quote}"</p>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="lg:pl-16">
            <div className="relative rounded-[2.5rem] overflow-hidden aspect-video sm:aspect-square bg-slate-100 border border-slate-100 shadow-xl">
              <img
                src={storeData?.aboutImageUrl || "https://images.unsplash.com/photo-1581578731548-c64695cc6952?auto=format&fit=crop&q=80&w=800"}
                referrerPolicy="no-referrer"
